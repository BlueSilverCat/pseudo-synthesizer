...
export default {
  ...
  activate() {
    ...
    isFileExist(this.path + '/data/source')
      .then((state) => {
        if(state === false) {
          readZip(this.path + '/data/source.zip')
  ...
  */
  ...
  playAudioData(audioData) {
    ...
    this.connectConvolverOrGain(source, this.activeConfigs.irIndex, true);
  ...
  makeSound(name) {
    ...
    this.connectConvolverOrGain(gainNode, this.activeConfigs.irIndex, true);
    for(let oscGain of oscGains ) {
      this.vibrator.connect(oscGain.oscillator.detune);
      oscGain.oscillator.start(this.audioContext.currentTime);
      oscGain.oscillator.stop(this.audioContext.currentTime + this.activeConfigs.sourceOscillatorLength);
    ...
    this.pianicRamp(gainNode);
  },

  pianicRamp(gainNode) {
    gainNode.gain.setValueAtTime(0, this.audioContext.currentTime);
    gainNode.gain.exponentialRampToValueAtTime(
      1, this.audioContext.currentTime + this.activeConfigs.sourceOscillatorLength * 0.2);
    gainNode.gain.exponentialRampToValueAtTime(
      0.5, this.audioContext.currentTime + this.activeConfigs.sourceOscillatorLength * 0.5);
    gainNode.gain.exponentialRampToValueAtTime(
      0.5, this.audioContext.currentTime + this.activeConfigs.sourceOscillatorLength * 0.8);
    gainNode.gain.exponentialRampToValueAtTime(
      0.01, this.audioContext.currentTime + this.activeConfigs.sourceOscillatorLength);
  },
  ...
  */
  ...
  startAutoPlay() {
    ...
    atom.notifications.addInfo(
      `Number of playable character : ${i}.\nPlay time : ` + (this.activeConfigs.autoPlayInterval * i).toString(10),
      {dismissable: true});
  },
  stopAutoPlay() {
  ...
  changePanner() {
    ...
    this.panner.distanceModel = 'linear'; //'linear', 'exponential', 'inverse';
    this.panner.refDistance = 1;  //減衰距離
    this.panner.maxDistance = 10000;
    this.panner.rolloffFactor = 0.1;  //減衰速度
    this.panner.coneInnerAngle = 30;
    this.panner.coneOuterAngle = 60;   //減衰する角度
    this.panner.coneOuterGain = 0.1;    //減衰率
  }
