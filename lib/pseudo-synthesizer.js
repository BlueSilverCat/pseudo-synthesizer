...
export default {
  config: {
    ...
    sourceOscillatorCustomWaveNormalize: {
      type: 'boolean',
      description: 'If set this to true, Source oscillator custom wave will be normalized',
      order: 13,
      'default': true
    },
  ...
  */
  ...
  setDefaultKeyBindConfigFilePath() {
    if (this.activeConfigs.japaneseKeybord === true) {
      this.activeConfigs.keyBindConfigFilePath = returnValueIfEmpty(this.activeConfigs.keyBindConfigFilePath, path.join(this.path, '/data/keybind-japanese.cson'));
    } else {
      this.activeConfigs.keyBindConfigFilePath = returnValueIfEmpty(this.activeConfigs.keyBindConfigFilePath, path.join(this.path, '/data/keybind.cson'));
    }
  },
