...
export function readZip(path, writePath) {
...
function checkDir(fileNames, writePath) {
  return (datas) => {
    return mkDir(writePath + fileNames[0]).then(write(fileNames, datas, writePath));
  };
}
...
function write(fileNames, datas, writePath) {
  return () => {
    ...
    for(let i = 1; i < fileNames.length; ++i) {
      promises.push(writeBinaryData(writePath + fileNames[i], datas[i]));
    }
