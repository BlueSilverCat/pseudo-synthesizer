...
export function readPeriodicWave(filePath) {
  if(filePath === null) {
    return null;
  }
  return createReadFilePromise(filePath, null, stringToPeriodicWave);
}
...
converter
*/
function stringToPeriodicWave(data) {
  ...
  for(let line of lines) {
    ...
    if(setNum(real, element[0]) === false) {
      break;
    }
    if(setNum(imag, element[1]) === false) {
      break;
    }
  }
...
function setNum(array, dataString) {
  let num = parseFloat(dataString);
  if(isNaN(num) === true) {
    return false;
  }
  array.push(num);
